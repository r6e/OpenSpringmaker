# Conical Compression Engine Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A new `springcore` family — round-wire conical compression springs (linear-range model, Shigley Prob. 10-29 rate) — plus additive `DesignSpec::Conical` persistence and the minimal springmaker placeholder arm.

**Architecture:** `springcore/src/conical/` mirrors the torsion family-module pattern: own inputs/design structs, `solve_forward`, `evaluate_status`. It REUSES the shared `LoadPoint`/`load_point` helper and `corrected_shear_stress` (stress governed by the largest coil), and the existing `EndType` solid-length/pitch formulas (conservative, non-telescoping). Persistence adds a third-level `ConicalSpec` enum under `DesignSpec::Conical`.

**Tech Stack:** Rust workspace — `springcore` (mutation-gated) + `springmaker` (iced GUI; this increment touches only one dispatch match).

## Global Constraints

- springcore is mutation-gated: `cargo mutants --in-diff` vs origin/main ends with literal 0 missed. springmaker is NOT gated.
- Strict TDD: failing test → watch it fail → implement → watch it pass.
- Every message quoted in this plan is VERBATIM — do not reword.
- NO references to the commercial spring-design product this project serves as an alternative to, nor its vendor. (Tooling attribution trailers are required and exempt.)
- MSRV 1.88; `cargo fmt` zero deviation; `cargo clippy --workspace --all-targets -- -D warnings` clean.
- Commit DIRECTLY on `feat/conical-engine`. NEVER push, NEVER create/edit PRs, NEVER run review panels or dispatch subagents, NEVER touch anything under `.git/`.
- Conventional commits; every commit ends with the trailer:
  `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`
- Do NOT add a `Conical` variant to `springcore::family::Family` — that enum drives the GUI tab list and belongs to the NEXT increment. This increment's GUI surface is ONE placeholder arm in `apply_saved`.
- Spec: docs/superpowers/specs/2026-07-06-conical-engine-design.md (Decisions 1–4 bind; the omissions in Decision 3 are deliberate — do not "helpfully" add natural frequency, buckling, or fatigue).

---

### Task 1: `springcore/src/conical/` — the engine

**Files:**
- Create: `springcore/src/conical/mod.rs`
- Create: `springcore/src/conical/design.rs`
- Modify: `springcore/src/lib.rs` (add `pub mod conical;` in alphabetical position, after `pub mod buckling;`-area — match the existing sorted module list)
- Modify: `springcore/src/design.rs` (two small shared changes, below)

**Interfaces:**
- Consumes: `crate::design::{LoadPoint, load_point, DesignStatus, StatusMessage, Severity, index_caution_labeled}`, `crate::end_type::EndType`, `crate::mechanics::{spring_index, corrected_shear_stress}`, `crate::material::Material`, `crate::CurvatureCorrection`, `crate::units::{Force, Length, SpringRate}`.
- Produces (Task 2 and the GUI increment rely on these exact names):
  `pub struct ConicalInputs { wire_dia, large_mean_dia, small_mean_dia, active_coils, free_length, end_type }`,
  `pub struct ConicalDesign { … }` (fields per Step 5),
  `pub fn solve_forward(material: &Material, inputs: &ConicalInputs, loads: &[Force], correction: CurvatureCorrection) -> Result<ConicalDesign>`,
  `pub fn evaluate_status(design: &ConicalDesign, material: &Material) -> DesignStatus>`.

- [ ] **Step 1: Two shared-file changes in `springcore/src/design.rs` (pure refactors — existing tests are the net)**

(a) Make the private `load_point` helper crate-visible so conical reuses it instead of duplicating (the function is family-agnostic given a mean diameter):

```rust
pub(crate) fn load_point(
```

(b) Split `index_caution` into a labeled core so conical can emit end-named cautions with the SAME band and message shape. Replace the existing function with:
...
The rendered message for existing families is byte-identical ("spring index {index:.2} is outside the recommended range 4–12").

- [ ] **Step 2: Run the existing suites to prove Step 1 is behavior-neutral**

Run: `cargo test -p springcore`
Expected: PASS (all existing tests, including the four index-boundary tests and every status-message test).

- [ ] **Step 3: Commit the shared groundwork**

```bash
git add springcore/src/design.rs
git commit -m "refactor(springcore): crate-visible load_point + labeled index_caution core

Conical (incoming) reuses both; rendered messages byte-identical.

Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"
```

- [ ] **Step 4: Write the failing conical tests**
...
Create `springcore/src/conical/design.rs` with ONLY the struct/function skeletons needed for the tests to compile-fail meaningfully is NOT the TDD style used in this repo — instead, write the test module first inside `design.rs` referencing the not-yet-written items, and watch the compile failure (the repo's established red step). Test module (append to `design.rs` once Step 5's code exists; for the red run, `design.rs` contains just the `use` lines and the test module):
...
mod tests {
    ...
    fn stress_is_computed_at_the_large_end() {
        ...
        assert!(
            (design.load_points[0].shear_stress.pascals() - at_small.pascals()).abs()
                > 1.0
        );
    }
    ...
    fn derived_diameters_indices_and_taper_are_exact() {
        let design = solve(20.0, 12.0).unwrap();
        assert_relative_eq!(design.large_outer_dia.millimeters(), 22.0, max_relative = 1e-9);
        assert_relative_eq!(design.large_inner_dia.millimeters(), 18.0, max_relative = 1e-9);
        assert_relative_eq!(design.small_outer_dia.millimeters(), 14.0, max_relative = 1e-9);
        assert_relative_eq!(design.small_inner_dia.millimeters(), 10.0, max_relative = 1e-9);
        ...
        assert_relative_eq!(design.taper_per_coil.millimeters(), 0.8, max_relative = 1e-9);
    ...
    fn guards_pin_messages_and_precedence() {
        ...
        i.active_coils = -1.0;
        assert_eq!(msg(run(&i)), "wire diameter must be a positive finite number");
        ...
        let mut i = base.clone();
        i.active_coils = 0.0;
        assert_eq!(msg(run(&i)), "active coils must be a positive finite number");
        let mut i = base.clone();
        ...
        assert_eq!(msg(run(&i)), "free length must be at least the solid length");
...
- [ ] **Step 5: Run to verify the tests fail to compile (missing items), then implement `design.rs`**

Run: `cargo test -p springcore conical`
Expected: compile FAIL — `ConicalInputs`/`solve_forward` etc. not found.

Then write the implementation ABOVE the test module in `springcore/src/conical/design.rs`:
...
fn conical_rate(shear_modulus: Stress, wire_dia: Length, large: Length, small: Length, active: f64) -> SpringRate {
...
pub fn solve_forward(
    ...
    let solid_length = inputs.end_type.solid_length(inputs.wire_dia, inputs.active_coils);
    ...
    let pitch = inputs
        .end_type
        .pitch_from_free_length(inputs.wire_dia, inputs.active_coils, inputs.free_length);
    ...
    let load_points: Vec<LoadPoint> = loads
        ...
        .collect();
    let solid_force =
        Force::from_newtons(rate.newtons_per_meter() * (l0 - solid_length.meters()));
...
Add to `springcore/src/lib.rs` (alphabetical among the existing `pub mod` lines):

```rust
pub mod conical;
```

- [ ] **Step 6: Run the conical suite to green, then the workspace**

Run: `cargo test -p springcore conical`
Expected: PASS (all tests from Step 4).
...
```bash
git add springcore/src/conical springcore/src/lib.rs
git commit -m "feat(conical): linear-range conical compression engine — Prob. 10-29 rate, governing-coil stress
...
```bash
git diff origin/main...HEAD > /tmp/conical-t1.diff
cargo mutants --in-diff /tmp/conical-t1.diff --package springcore
```
Expected: `… 0 missed`. Survivors get killing tests (never reclassified); note that the Step-1 refactor puts `load_point`/`index_caution_labeled` in the diff — their mutants must die under the existing + new tests.

---

### Task 2: Persistence + springmaker placeholder arm

**Files:**
- Modify: `springcore/src/persistence.rs` (DesignSpec + ConicalSpec + solve_with_material rejection arm + tests)
- Modify: `springmaker/src/app.rs` (`apply_saved` placeholder arm + one test)

**Interfaces:**
- Consumes: Task 1's `ConicalInputs`/`solve_forward` are NOT consumed here — `solve_with_material` REJECTS conical (the existing extension/torsion pattern; families solve via their own GUI paths). Consumes `DesignSpec`/`SavedDesign` conventions from persistence.rs and `self.action_error` from app.rs.
- Produces: `ConicalSpec::PowerUser { end_type: String, wire_dia_mm: f64, large_mean_dia_mm: f64, small_mean_dia_mm: f64, active: f64, free_length_mm: f64, loads_n: Vec<f64> }` and `DesignSpec::Conical(ConicalSpec)` — the GUI increment builds on these exact names.

- [ ] **Step 1: Write the failing persistence tests**

Append to the tests module in `springcore/src/persistence.rs`, following the file's existing round-trip/reject test conventions:
...
fn from_toml_rejects_non_finite_conical_float() {
    let saved = SavedDesign {
        ...
        design: DesignSpec::Conical(ConicalSpec::PowerUser {
            end_type: "SquaredGround".to_string(),
            wire_dia_mm: 2.0,
            large_mean_dia_mm: f64::INFINITY,
            small_mean_dia_mm: 12.0,
            active: 10.0,
            free_length_mm: 60.0,
            loads_n: vec![10.0],
...
(Adapt the `SavedDesign` construction to the file's existing test helpers if it uses a builder; mirror the neighboring `solve_with_material_rejects_torsion_design` test's exact shape. NOTE on the long string: the `\`-continuation collapses to a single space — the expected message is one line: `"SavedDesign::solve handles compression designs; conical designs are solved via the conical scenario"`.)

- [ ] **Step 2: Run to verify they fail to compile**

Run: `cargo test -p springcore persistence`
Expected: compile FAIL — `ConicalSpec` / `DesignSpec::Conical` not found.

- [ ] **Step 3: Implement the persistence surface**

In `springcore/src/persistence.rs`:

(a) Add the variant to `DesignSpec` (after `Torsion`):

```rust
    Conical(ConicalSpec),
```

(b) Add the spec enum (after `TorsionSpec`, same conventions — internally tagged, every field required so a misspelled key errors as missing):
...
(c) Add the rejection arm in `solve_with_material` (after the `DesignSpec::Torsion(_)` arm, same message pattern):
...
(d) Export `ConicalSpec` wherever `TorsionSpec` is exported (`lib.rs`'s `pub use persistence::{...}` list).

`reject_non_finite` walks the whole `toml::Value` tree, so the new variant is covered with no changes.

- [ ] **Step 4: The springmaker placeholder arm + test**

In `springmaker/src/app.rs`, `apply_saved`'s `match saved.design` gains (after the `Torsion` arm):
...
Add a test to app.rs's test module (mirror the neighboring apply/load test conventions):

```rust
#[test]
fn loading_a_conical_design_surfaces_a_clean_action_error() {
    let mut app = App::default(); // match the module's established constructor
    ...
    assert!(err.contains("conical designs are not supported"), "got: {err}");
}
```

(Adapt the `App` construction and `apply_saved` visibility to the module's actual test conventions — read the neighboring tests first; if `apply_saved` is private and untested directly, drive it through the module's established load path instead.)

- [ ] **Step 5: Run everything to green**

Run: `cargo test --workspace`
Expected: PASS. Then `cargo clippy --workspace --all-targets -- -D warnings && cargo fmt --all --check` — clean.

- [ ] **Step 6: Commit**

```bash
git add springcore/src/persistence.rs springcore/src/lib.rs springmaker/src/app.rs
git commit -m "feat(conical): additive DesignSpec::Conical persistence + GUI placeholder arm
...
In order, unmasked exit codes (`typos` bare, no pipes):
...
typos
git diff origin/main...HEAD > /tmp/conical-full.diff
cargo mutants --in-diff /tmp/conical-full.diff --package springcore
```
